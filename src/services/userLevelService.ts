...
export class UserLevelService {
  ...
  static getOrCreateUserLevel(userId: number, levelId: number): UserLevel {
    ...
    if (!userLevel) {
      ...
      const levelInfo = db.prepare('SELECT level_number FROM levels WHERE id = ?').get(levelId) as
        | { level_number: number }
        | undefined;
      const isUnlocked =
        levelInfo?.level_number === 1 ||
        this.checkPreviousLevelMastery(userId, levelInfo?.level_number || 1);
  ...
  static updateAllLevelMastery(
    ...
    if (sessionLevelId) {
      const sessionLevel = db
        .prepare('SELECT level_number FROM levels WHERE id = ?')
        .get(sessionLevelId) as { level_number: number } | undefined;
    ...
    for (const level of levels) {
      ...
      const wasNewlyHit =
        previousMastery < 80 && mastery >= 80 && attempts >= 6 && !currentUserLevel?.mastery_hit;
